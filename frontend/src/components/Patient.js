...
import { Calendar, Clock, FileText, User, Users, ChevronDown, Home, UserCircle, Calendar as CalendarIcon, Eye, EyeOff, Hospital , History,HeartPulse} from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import TransactionDetails from "./TransactionDetails";

import axios from 'axios';
const MONGODB_API = "http://127.0.0.1:5000"; // Replace with your actual MongoDB API endpoint
...
export default function PatientDashboard() {
  ...
  const HeartDiseasePredictor = () => {
    return (
      <div>
        ...
        <iframe
          src={`http://localhost:5173/?email=${encodeURIComponent(patientInfo?.email)}`} // Pass email as URL parameter
          width="100%"
          height="600px"
    ...
    <div className="min-h-screen bg-blue-600">
      ...
      <nav className="bg-blue-700 text-white p-4">
        <ul className="flex space-x-4 justify-center">
          ...
          <li>
            <Button
              variant={activeTab === 'Heart Disease Predictor' ? "outline" : "ghost"}
              className={`hover:bg-white hover:text-blue-600 ${activeTab === 'Heart Disease Predictor' ? 'bg-white text-blue-600' : 'text-white'}`}
              onClick={() => setActiveTab('Heart Disease Predictor')}
          ...
          <li>
            <Button
              variant={activeTab === 'Past Health History' ? "outline" : "ghost"}
              className={`hover:bg-white hover:text-blue-600 ${activeTab === 'Past Health History' ? 'bg-white text-blue-600' : 'text-white'}`}
              onClick={() => setActiveTab('Past Health History')}
  ...
  {activeTab === 'Heart Disease Predictor' && <HeartDiseasePredictor />}
  {activeTab === 'Past Health History' && (
